...
const char stopParticleMeasurement[4] = {0x68,0x01,0x02,0x95};
  ...
  //단순한 수신 알고리즘 ---- (최종적으로는 사용하면 안됨) 
	   if(read(client,&rxSensorResponse.rxByte,8)==8){
 		   PM10 = rxSensorResponse.resp.dataField[2]*256 + rxSensorResponse.resp.dataField[3];
		  	   printf("PM10 = %d \n", PM10);
		   PM2dot5 = rxSensorResponse.resp.dataField[1]*256 + rxSensorResponse.resp.dataField[4];
			  printf("PM2.5 = %d \n", PM2dot5);
			   for(int i=0; i<8; i++){
     ...
     // ----------------------------------------
      ...
      if(write(client, stopParticleMeasurement, sizeof(stopParticleMeasurement))<0){
                  perror("Error: Failed to write to the sensor\n");
                  return -1;
           }
